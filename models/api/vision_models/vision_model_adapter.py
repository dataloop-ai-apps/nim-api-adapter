...
class ModelAdapter(dl.BaseModelAdapter):
    ...
    def extract_annotations_yolox(self, img, image_b64, collection):
        ...
        for box in response.get('data', []): # todo try with 2 annotations
            label = next(iter(box['bounding_boxes']))
            values = box['bounding_boxes'][label][0]
            collection.add(annotation_definition=dl.Box(left=max(values['x_min']*img.shape[1], 0),
                                                            top=max(values['y_min']*img.shape[0], 0),
                                                            right=min(values['x_max']*img.shape[1], img.shape[1]),
                                                            bottom=min(values['y_max']*img.shape[0], img.shape[0]),
                                                            label=label
                                                            ),
